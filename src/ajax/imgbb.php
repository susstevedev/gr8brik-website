...
class ImgBB {
    ...
    public function upload(array $image) {
        ...
        if (isset($res['success']) && $res['success'] === true) {
            ...
            $stmt = $this->db->prepare("INSERT INTO attachments (userid, url, bbcode, mime, delete_url) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("issss", $user, $url, $bbcode, $mime, $delete_url);
            ...
            return ['success' => false, 'message' => $res['error']['message'] ?? '[unknown error]'];
        }
    }

    /**
     * Returns an attachment from it's uuid
     */
    public function get(int $id) {
        ...
        if (isset($row)) {
            ...
            return [
                ...
                'id' => $id,
                'image' => [
                    'user' => $usero->username ?? null,
                    'mime' => $row['mime'] ?? null,
                    'url' => $row['url'] ?? null,
                    'bbcode' => $row['bbcode'] ?? null,
                    'remove' => $delete_url ?? null
                ]
            ];
        } else {
