...
$sql = "SELECT id, user, model, description, name, date, screenshot FROM model WHERE id = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("i", $model_id);
$stmt->execute();
		
$stmt->bind_result($model_id, $userid, $model, $description, $name, $date, $screenshot);
$stmt->store_result();
$stmt->fetch();
...
if(isset($_POST['comment'])){
        ...
        $notifName = " commented on your model!";
...
<html lang="en">
<head>
    <title>Gr8brik.rf.gd | <?php echo $name ?></title>
...
<?php 
    ...
    echo '<h4><i class="fa fa-user-o" aria-hidden="true"></i>By <a href="/@' . urlencode($username) . '">' . $username . '</a>';
    echo '</h4>';

    echo '<h4><i class="fa fa-arrow-circle-o-up" aria-hidden="true"></i>' . $count_row['vote_count'] . ' votes</h4>';
    if(!empty($decoded_description)) {
        echo '<pre class="w3-card-2 w3-light-grey w3-padding-tiny">' . $bbcode->toHTML($decoded_description) . '</pre><br />';
    }
...
?>
		...
		if(isset($_SESSION['username'])) {
            ...
            if ($result2->num_rows > 0) {
                ...
                echo '<input form="upvote" class="w3-btn w3-blue w3-hover-white" type="submit" value="Vote" name="upvote">&nbsp;&nbsp';
            }
			...
			echo "<!-- <br /><b>Embed:<input type='text' class='w3-input w3-mobile' value='" . 'http://www.gr8brik.rf.gd/embed.php?model=' . $model . '&login=' . $_SESSION['status'] . "' style='width:auto;height:50px;'></b> -->";

            echo "<br /><br /><div id='commentForm'>";
            echo "<div id='post'>";
            echo "<img id='pfp-post' src='" . '../acc/users/pfps/' . $id . '.jpg' . "'>";
			echo "<textarea name='commentBox' id='commentBox' class='w3-mobile' placeholder='Add a comment...' rows='4' cols='40'></textarea></div><br />";
			...
			echo "<br /><button id='postComment' class='w3-btn w3-blue w3-hover-white'>Comment</button></div>";
		} else {
			...
			$sql = "SELECT * FROM comments WHERE model = $model_id ORDER BY date ASC";
			...
			while ($row = $comResult->fetch_assoc()) {
                ...
                echo '<div id="post">';
                echo '<img id="pfp-post" src="/acc/users/pfps/' . $c_user . '.jpg">';
                echo '<article class="w3-card-2 w3-light-grey w3-padding-tiny" style="width:50%">';
                echo '<header><a href="/@' . urlencode($userRow['username']) . '"><b>' . $userRow['username'] . '</b></a>';
                echo '<time class="w3-right w3-text-grey" datetime="' . $row['date'] . '">' . $row['date'] . '</time></header>';
                echo '<pre>' . $bbcode->toHTML(htmlentities($row['comment'], ENT_QUOTES, 'UTF-8')) . '</pre>';
                echo '</article></div><br />';
			}
