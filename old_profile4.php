...
<script>
        ...
        $.ajax({
                ...
                success: function(response) {
                    ...
                    if(response.twitter !== null && response.twitter !== undefined && response.twitter !== '') {
                        ...
                        $("#twitter-link").attr("href", `https://twitter.com/${response.twitter}`);
                    }
                    ...
                    if(isLoggedin === true) {
                        if(userid != response.tokenuser) {
                            ...
                            $("#data-edit-profile").show();
                        }
                        $("#data-user-actions").show();
                    } else {
                        $("#followedby-wrapper").hide();
                        $("#twitter-wrapper").hide();
                        $("#data-user-actions").remove();

                        if(response.message != "") {
                            console.log(response.message);
                            $("#data-profile-message").show().text(response.message);
                        }
                    }
                    ...
                    $("#joined").text(response.age);

                    if(response.description) {
                        $("#description").html(response.description).text();
                    } else {
                        $("#description").hide();
                    }
                },
                error: function(jqXHR, textStatus, errorThrown) {
                    ...
                    $("#data-profile-error").show();
                    $("#data-error-code").text(jqXHR.status);
                    $("#data-error-text").text(errorThrown + ' ' + textStatus);
                }
            });
            setTimeout(() => {
                $.ajax({
    ...
    <div class='w3-light-grey w3-card-2 gr8-theme w3-padding-small'>
    <center>
    ...
    <span style='font-size: 30px; display: inline-block; vertical-align: top; max-width: 100%;'>
        ...
        <b id="username"></b>
        <span id="data-verified" style="display: none;"><i class="fa fa-check w3-blue w3-xlarge"></i></span>
        <span id="data-admin" style="display: none;"><i class="fa fa-check w3-red w3-xlarge"></i></span>
        <span id="data-edit-profile" style="display: none;"><a href="/acc/index" class="reload"><i class="fa fa-pencil w3-xlarge" aria-hidden="true"></i></a></span>
    </span>
    
    <b style="font-size:20px;">
        <span style="text-decoration: underline dotted;"><span id="data-model-count"></span>&nbsp;creations</span>
        <span style="text-decoration: underline dotted;"><span id="data-follower-count"></span>&nbsp;followers</span>
        <span style="text-decoration: underline dotted;"><span id="data-following-count"></span>&nbsp;following</span>
    </b>

    <pre id="description"></pre><hr />

    <span style="font-size: 15px; text-shadow: 0px 0px 0px #fff;">Joined <span id="joined"></span></span>
    <span id="twitter-wrapper" style="display: none; font-size: 15px; text-shadow: 0px 0px 0px #fff;">
        &nbsp;|&nbsp;<a id="twitter-link" href="https://twitter.com/null" target="_blank"><span class="w3-large fa fa-twitter"></span><span id="twitter"></span></a>
    </span>
    <span id="followedby-wrapper" style="font-size: 15px; text-shadow: 0px 0px 0px #fff;">&nbsp;|&nbsp;Followed by <span id="followedby"></span></span><br />

    <form id="followUser" action="" method="post"></form>
    <span id="action-buttons" style="display: none;">
        <button id="button-unfollow" onclick="document.getElementById('unfollow').style.display='block'" name='unfollow' class='w3-btn w3-red w3-hover-opacity w3-round-small w3-padding-small w3-border w3-border-pink' />Unfollow</button>&nbsp;
        ...
        <button id="button-ban-user" onclick=document.getElementById('ban').style.display='block' name='ban' class='w3-btn w3-red w3-hover-opacity w3-round-small w3-padding-small w3-border w3-border-pink' />Ban</button>&nbsp;
    </span>
            ...
            ?>
            <center>
                <span id="data-profile-error" style="display: none; text-align: center;">
                    <i class="w3-xlarge fa fa-exclamation-triangle" aria-hidden="true"></i><br />
                    <span id="data-error-code" class="w3-xlarge"></span><br />
                    <span id="data-error-text" class="w3-text-grey w3-large"></span>
                </span>

                <span id="data-profile-message" class="w3-large w3-text-grey" style="display: none;"></span>
            </center>
